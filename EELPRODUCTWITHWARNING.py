...
running = True
cycle_count = 0
device_active = False  # Track if the device (e.g., fan/pump) is active
consecutive_breaches = 0  # Track consecutive threshold breaches
warning_status = "Normal"  # Device status for warning
...
def notify_nodemcu(exceeded: bool):
    ...
    global device_active, consecutive_breaches, warning_status
    try:
        ...
        elif exceeded and device_active:
            consecutive_breaches += 1
            if consecutive_breaches >= 30:
                warning_status = "Warning: Pump may not be working or technical problem"
                logger.warning("Temperature above threshold for 30 consecutive measurements. Possible pump or technical issue.")
            response = ser.readline().decode('utf-8').strip()
            if response:
                logger.info(f"NodeMCU response: {response}")
...
@app.route('/data')
def get_data():
    global sensor_data, cycle_data, cycle_count, warning_status
    return jsonify({
        'cycle': cycle_count,
    ...
    <main>
        ...
        <section class="data-section" aria-label="Temperature data">
            <div class="metrics">
                ...
                <div class="metric-card" id="warning-card">
        ...
        function fetchData() {
            fetch('/data')
                ...
                .then(data => {
                    ...
                    warningStatus.textContent = data.warning_status;
                    if (data.warning_status !== "Normal") {
