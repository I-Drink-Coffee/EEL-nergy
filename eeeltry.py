...
def notify_nodemcu(exceeded: bool):
    global device_active, pump_on_time
    try:
...
def gather_data() -> List[float]:
    global cycle_count, last_temp, no_change_counter, pump_warning, pump_on_time
    temperatures = []
    ...
    try:
        while time.time() - start_time < 600:
            ...
            current_avg = np.mean(temp_grid)
            temperatures.append(current_avg)

            if pump_on_time:
                if last_temp is not None and abs(current_avg - last_temp) < 0.3:
                    no_change_counter += 1
                else:
                    no_change_counter = 0

                if no_change_counter >= 300:
                    pump_warning = True
                else:
                    pump_warning = False

            last_temp = current_avg

            if current_avg > TEMP_THRESHOLD:
                logger.warning(f"Temperature exceeded threshold: {current_avg:.2f}°C")
                notify_nodemcu(True)
            else:
    ...
    with open(CSV_FILE, 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([time.strftime('%Y-%m-%d %H:%M:%S'), cycle_count, f"{avg_temp:.2f}", "", "Yes" if avg_temp > TEMP_THRESHOLD else "No"])
    return temperatures
...
@app.route('/data')
def get_data():
    global sensor_data, cycle_data, cycle_count, pump_warning
    return jsonify({
        'cycle': cycle_count,
...
<body>
    ...
    <span id="pump-warning" style="color: red; font-weight: bold; display: none;">
        ⚠️ Warning: Pump may not be functioning properly.
    </span>
</div>
    ...
    fetch('/data')
        ...
        .then(data => {
            ...
            document.getElementById('pump-warning').style.display = data.pump_warning ? 'block' : 'none';
